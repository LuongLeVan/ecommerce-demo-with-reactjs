const img1 = require("../images/slider/slide_1.png")
const img2 = require("../images/apn3890_vag__2_.png")
const img3 = require("../images/slider/slide_3.png")

const heroSliderData = [
    {
        title: "Polo nữ Pima Lascope",
        description: "Nhắc đến sự đẳng cấp là không thể không nhắc đến dòng vải pima. Nó tạo nên chất lượng tốt cho bất kỳ sản phẩm thời trang. Sợi vải pima dày dặn.",
        img: img1,
        color: "blue",
        path: "/catalog/ao-thun-dinosaur-01"
    },
    {
        title: "Polo Nữ Dáng Suông Modal",
        description: "Polo nữ dáng suông Modal sử dụng công nghệ vải cao cấp thân thiện với môi trường sản xuất độc quyền chống co rút vải, áo polo nữ vải modal.",
        img: img2,
        path: "/catalog/ao-thun-dinosaur-02",
        color: "pink"
    },
    {
        title: "Polo Nữ Coolmax Lacoste",
        description: "Mẫu áo polo nữ được làm bằng chất liệu coolmax đem lại cảm giác mát lạnh khi mặc. Thiết kế mẫu áo polo coolmax này có kiểu dáng cực kỳ thoải mái.",
        img: img3,
        path: "/catalog/ao-thun-dinosaur-03",
        color: "orange"
    }
]

export default heroSliderData